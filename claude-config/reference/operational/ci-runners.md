...
|---|---|---|---|---|
| mac-worker-1-runner | `<mac-worker-1>` (`<mac-worker-1-user>`) | online | ~/actions-runner | self-hosted, macOS, ARM64 |
| mac-worker-1-runner-2 | `<mac-worker-1>` (`<mac-worker-1-user>`) | online | ~/actions-runner-2 | self-hosted, macOS, ARM64 |
| mac-worker-1-runner-3 | `<mac-worker-1>` (`<mac-worker-1-user>`) | online | ~/actions-runner-3 | self-hosted, macOS, ARM64 |
| linux-runner-1 | `<linux-worker>` (`<linux-worker-user>`) | online | ~/actions-runner | self-hosted, Linux, X64 |
| mac-runner-1 | `<coordinator>` (`<coordinator-user>`) | offline | ~/actions-runner | self-hosted, macOS, ARM64 |
...
_Why: Plan 387 — 3 runners on mac-worker-1 all hit `EEXIST` during concurrent `npm ci` until npm caches were isolated._
