...
cmd_import() {
	...
	if [[ -z "$existing_desc" || "$existing_desc" == "{" ]]; then
		local spec_desc
		# jq -r already outputs one line; avoid pipe to head (SIGPIPE with pipefail)
		spec_desc=$(jq -r '.description // .user_request // empty' "$effective_spec" 2>/dev/null)
		spec_desc="${spec_desc%%$'\n'*}"
		spec_desc="${spec_desc:0:200}"
		if [[ -n "$spec_desc" ]]; then
  ...
  if (.model // "") | test("^gpt-|^codex") then "copilot"
	...
	for ((i = 0; i < wave_count; i++)); do
		...
		for ((j = 0; j < task_count; j++)); do
			...
			if [[ "$has_do" == "true" ]]; then
				...
				t_criteria=$(echo "$spec_data" | jq -c "if $t_base.test_criteria != [] then $t_base.test_criteria elif $t_base.verify != [] then $t_base.verify else [] end")
			fi
