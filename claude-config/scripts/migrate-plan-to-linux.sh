...
# --- Detect Linux worker host ---
_linux_worker_host() {
    [[ -z "${NODE_LINUX_ALIAS_LOCAL:-}" ]] && { echo "ERROR: NODE_LINUX_ALIAS_LOCAL not set. Run: cp config/local.env.example config/local.env" >&2; return 1; }
    [[ -z "${NODE_LINUX_ALIAS_TS:-}" ]] && { echo "ERROR: NODE_LINUX_ALIAS_TS not set. Run: cp config/local.env.example config/local.env" >&2; return 1; }
    local _local="${NODE_LINUX_ALIAS_LOCAL}"
    local _ts="${NODE_LINUX_ALIAS_TS}"
    if ssh -o ConnectTimeout=2 -o BatchMode=yes "$_local" true 2>/dev/null; then
        echo "$_local"
    elif ssh -o ConnectTimeout=3 -o BatchMode=yes "$_ts" true 2>/dev/null; then
        echo "$_ts"
    else
        return 1
    fi
...
HOST=$(_linux_worker_host) || {
    err "Linux worker unreachable (tried ${NODE_LINUX_ALIAS_LOCAL} and ${NODE_LINUX_ALIAS_TS})"
    exit 1
}
...
REMOTE_LINUX_USER="${REMOTE_LINUX_USER:-$USER}"
LINUX_REPO="/home/${REMOTE_LINUX_USER}/GitHub/Convergio"
LINUX_WORKTREE="/home/${REMOTE_LINUX_USER}/GitHub/$WORKTREE_BASENAME"
...
ssh "$HOST" bash -s -- "$PLAN_ID" "$LINUX_WORKTREE" "${NODE_LINUX_ALIAS_TS:-linux-worker}" <<'REMOTE_DB'
set -euo pipefail
PLAN_ID="$1"
LINUX_WT="$2"
EXECUTION_HOST="$3"
