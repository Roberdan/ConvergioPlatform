...
REPO_ROOT="${REPO_ROOT:-$HOME/GitHub/MirrorBuddy}"

[[ -z "${AUDIT_REMOTE:-}" ]] && { echo "ERROR: AUDIT_REMOTE not set. Run: cp config/local.env.example config/local.env" >&2; exit 1; }
REMOTE="${AUDIT_REMOTE}"
...
scp "${REPO_ROOT}/.claude/plans/mirrorbuddy-audit-remediation-spec.yaml" \
...
scp "${REPO_ROOT}/.claude/prompts/audit-remediation-2026-02-28.md" \
