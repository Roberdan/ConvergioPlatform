...
# Version: 1.6.0
DB_FILE="${DASHBOARD_DB:-${HOME}/.claude/data/dashboard.db}"
...
_find_capable_sqlite3() {
	# Cache: reuse previously found path (persists across script invocations)
	local cache="${TMPDIR:-/tmp}/sqlite3-capable-path"
	if [[ -f "$cache" ]]; then
		local cached; cached="$(cat "$cache" 2>/dev/null)"
		[[ -x "$cached" ]] && { echo "$cached"; return; }
	fi
	...
	for p in /opt/homebrew/opt/sqlite/bin/sqlite3 /usr/local/opt/sqlite/bin/sqlite3; do
		[[ -x "$p" ]] && { echo "$p" > "$cache"; echo "$p"; return; }
	done
	...
	if [[ -n "$sys" ]]; then
		...
		if "$sys" ":memory:" ".load $_CRSQLITE_EXT" "SELECT 1;" &>/dev/null; then
			echo "$sys" > "$cache"; echo "$sys"; return
		fi
	fi
	# 3. Rust binary as fallback (claude-core db)
	local result="${sys:-sqlite3}"
...
db_query() {
	sqlite3 -cmd ".timeout 5000" \
		...
		-cmd "PRAGMA mmap_size = 268435456" \
		"$DB_FILE" "$@"
}
...
load_sync_config() {
	...
	# Set defaults (DEPLOY_PEER from local.env takes precedence)
	[[ -z "${REMOTE_HOST:-}" && -z "${DEPLOY_PEER:-}" ]] && { echo "ERROR: DEPLOY_PEER not set. Run: cp config/local.env.example config/local.env" >&2; return 1; }
	REMOTE_HOST="${REMOTE_HOST:-${DEPLOY_PEER}}"
