...
publish_task_status_event() {
	...
	local to_status="$3"
	local has_ipc
	has_ipc=$(sqlite3 "$DB_FILE" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='ipc_messages';" 2>/dev/null || echo "0")
	[[ "$has_ipc" -eq 0 ]] && return 0

	local content_json content_sql msg_id
	content_json=$(sqlite3 "$DB_FILE" \
		"SELECT json_object(
			'event','task_status_changed',
			'task_db_id',id,
			'task_id',task_id,
			'plan_id',plan_id,
			'wave_db_id',wave_id_fk,
			'from_status','$from_status',
			'to_status','$to_status',
			'timestamp',strftime('%Y-%m-%dT%H:%M:%fZ','now')
		) FROM tasks WHERE id = $task_db_id;" 2>/dev/null || echo "")
	[[ -z "$content_json" ]] && return 0

	content_sql=$(printf "%s" "$content_json" | sed "s/'/''/g")
	msg_id="task-status-${task_db_id}-${to_status}-$(date +%s)-$RANDOM"
	sqlite3 -cmd ".timeout 3000" "$DB_FILE" \
		"INSERT INTO ipc_messages (id, from_agent, to_agent, channel, content, msg_type, priority)
		 VALUES ('$msg_id', 'plan-db-safe', 'thor', 'plan.task.status', '$content_sql', 'task_status_changed', 5);" 2>/dev/null || true
}
...
if [[ "$COMMAND" == "update-task" && "$STATUS" == "done" ]]; then
	...
	status_before_submit=$(sqlite3 -cmd ".timeout 3000" "$DB_FILE" \
		"SELECT status FROM tasks WHERE id = $TASK_ID;" 2>/dev/null || echo "in_progress")
	PLAN_DB_SAFE_CALLER=1 "$SCRIPT_DIR/plan-db.sh" "update-task" "$TASK_ID" "submitted" "${@:4}"
	publish_task_status_event "$TASK_ID" "${status_before_submit:-in_progress}" "submitted"
