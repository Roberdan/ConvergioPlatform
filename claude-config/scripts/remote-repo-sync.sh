#!/usr/bin/env bash
# remote-repo-sync.sh - Sync repos and check CLI versions on the Linux worker
...
# URLs use config vars where available; fallback to known defaults.
_GH="${GH_DEFAULT_ACCOUNT:-Roberdan}"
[[ -z "${GH_MICROSOFT_ACCOUNT:-}" ]] && { echo "ERROR: GH_MICROSOFT_ACCOUNT not set. Run: cp config/local.env.example config/local.env" >&2; exit 1; }
_GH_MS="${GH_MICROSOFT_ACCOUNT}"
REPOS=(
