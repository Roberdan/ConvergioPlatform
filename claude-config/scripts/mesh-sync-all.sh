...
phase_repos() {
	...
	# Build list of online peers with their SSH destinations (parallel connectivity check)
	local -a online_peers=() online_dests=()
	local offline=0
	local -a all_peers=() all_dests=() check_pids=()
	local tmpdir_check="${TMPDIR:-/tmp}/mesh-sync-check-$$"
	...
	while IFS= read -r peer <&3 || [[ -n "$peer" ]]; do
		...
		all_peers+=("$peer")
		all_dests+=("$dest")
		( ssh -n -o ConnectTimeout=5 -o BatchMode=yes "$dest" true 2>/dev/null && echo "OK" || echo "FAIL" ) > "$tmpdir_check/$peer" &
		check_pids+=($!)
	done 3< <(get_targets)

	for pid in "${check_pids[@]}"; do wait "$pid" 2>/dev/null || true; done

	for idx in "${!all_peers[@]}"; do
		local status; status=$(cat "$tmpdir_check/${all_peers[$idx]}" 2>/dev/null)
		if [[ "$status" == "OK" ]]; then
			online_peers+=("${all_peers[$idx]}")
			online_dests+=("${all_dests[$idx]}")
		else
			echo -e "  ${Y}${all_peers[$idx]}${N}: OFFLINE"
			((offline++)) || true
		fi
	done
	rm -rf "$tmpdir_check"
...
phase_verify() {
	...
	# Build the remote command once (same for all peers)
	local verify_cmd=""
	...
	local tmpdir_verify="${TMPDIR:-/tmp}/mesh-verify-$$"
	mkdir -p "$tmpdir_verify"
	local -a verify_peers=() verify_pids=()
	...
	while IFS= read -r peer <&3 || [[ -n "$peer" ]]; do
		...
		(
			if ! ssh -n -o ConnectTimeout=5 -o BatchMode=yes "$dest" true 2>/dev/null; then
				echo "OFFLINE" > "$tmpdir_verify/$peer"
			else
				ssh -n -o ConnectTimeout=10 "$dest" "$verify_cmd" > "$tmpdir_verify/$peer" 2>/dev/null || echo "???" > "$tmpdir_verify/$peer"
			fi
		) &
		verify_pids+=($!)
	done 3< <(get_targets)
	...
	for peer in "${verify_peers[@]}"; do
		printf "  %-12s" "$peer"
		local result_file="$tmpdir_verify/$peer"
		if [[ -f "$result_file" ]] && grep -q '^OFFLINE$' "$result_file" 2>/dev/null; then
