...
# Ahead/behind tracking branch — combine into single read
...
# Status counts (one git status call, parsed with single awk — no grep subshells)
...
# Recent commits — build JSON array without jq -R (saves ~50ms)
COMMITS="["
while IFS= read -r _cline; do
	[[ -n "$_cline" ]] && COMMITS="${COMMITS}\"${_cline//\"/\\\"}\","
done < <(git log --oneline -5 2>/dev/null)
COMMITS="${COMMITS%,}]"
[[ "$COMMITS" == "]" ]] && COMMITS="[]"

# Stash count (bash builtin count — no grep subshell)
STASH_COUNT=0
while IFS= read -r _; do STASH_COUNT=$((STASH_COUNT + 1)); done < <(git stash list 2>/dev/null)
