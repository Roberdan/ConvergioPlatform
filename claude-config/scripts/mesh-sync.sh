...
_sync_peer() {
  ...
  # Primary: direct SSH push (always works, no GitHub auth needed on peers)
  local route
  route="$(peers_best_route "$peer")"
  _ssh_cmd "$peer" "cd ~/.claude && git stash -q 2>/dev/null || true" 2>/dev/null
  cd ~/.claude && git push "${route}:~/.claude" "main:main-sync" --force -q 2>&1 && \
    _ssh_cmd "$peer" "cd ~/.claude && git checkout main-sync -q 2>/dev/null; git branch -M main-sync main; git checkout main -q" 2>&1 || {
    if $FORCE; then
      # Fallback: try origin remote on peer (requires GitHub auth)
      echo "    direct push failed, trying origin fetch on peer..."
      _ssh_cmd "$peer" "cd ~/.claude && git fetch origin main -q && git checkout main -q 2>/dev/null && git reset --hard origin/main -q" 2>&1
    else
