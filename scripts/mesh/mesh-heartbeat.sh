...
_check_plan_events() {
	local peer_name
	peer_name="$(peers_self 2>/dev/null || echo "$(hostname -s 2>/dev/null || hostname)")"
	while IFS='|' read -r plan_id plan_name _tasks_done tasks_total; do
		[[ -z "$plan_id" ]] && continue
		local resolved
	...
	while IFS='|' read -r wave_id wave_name plan_id _wd wt; do
		[[ -z "$wave_id" ]] && continue
		local wave_resolved
...
_daemon_loop() {
	...
	while true; do
		...
		# Every beat: retry pending rsync patches from post-commit-sync (non-fatal)
		local sync_script="$SCRIPT_DIR/post-commit-sync.sh"
		if [[ -x "$sync_script" ]]; then
			local repo_root
			repo_root="$(git rev-parse --show-toplevel 2>/dev/null || echo "")"
			if [[ -n "$repo_root" && -d "$repo_root/.git/pending-patches" ]]; then
				"$sync_script" --repo-path "$repo_root" --retry-pending \
					>>"$CLAUDE_HOME/data/mesh-heartbeat.log" 2>&1 &
			fi
		fi
