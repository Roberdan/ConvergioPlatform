...
mod tests {
    ...
    fn hooks_dispatch_denies_main_branch_git_write() {
        ...
        let output =
            dispatch_pre_tool(&payload_for("git commit -m test"), &context).expect("dispatch");
        let reason = output
    ...
    fn hooks_dispatch_returns_gh_token_env() {
        ...
        context
            .gh_tokens
            .insert("robot".to_string(), "secret".to_string());
        let output =
            dispatch_pre_tool(&payload_for("gh pr view 1 --json id"), &context).expect("dispatch");
        let token = output
    ...
    fn hooks_dispatch_blocks_stale_execution_preflight() {
        ...
        let output =
            dispatch_pre_tool(&payload_for("plan-db.sh start 100025"), &context).expect("dispatch");
        let reason = output
            .and_then(|value| value.get("permissionDecisionReason").cloned())
            .and_then(|value| value.as_str().map(str::to_owned))
            .expect("deny reason");
    ...
    fn hooks_dispatch_reuses_single_sqlite_connection() {
        ...
        let mut context = CheckContext::for_tests();
        context.db_path = db;
        dispatch_pre_tool(&payload_for("az containerapp create -n demo"), &context)
            .expect("dispatch 1");
