...
// SqliteMemoryStore lives in super (sqlite_store), types and MemoryStore in super::super (memory).
...
fn recall_filters_by_type() {
    ...
    let results = store
        .recall(RecallQuery { memory_type: Some(MemoryType::Fact), limit: 10, ..Default::default() })
        .expect("recall");
    assert_eq!(results.len(), 1);
...
fn recall_filters_by_tags() {
    ...
    let results = store
        .recall(RecallQuery { tags: Some(vec!["rust".to_string()]), limit: 10, ..Default::default() })
        .expect("recall");
...
fn recall_filters_by_agent_id() {
    ...
    let results = store
        .recall(RecallQuery { agent_id: Some("agent-one".to_string()), limit: 10, ..Default::default() })
        .expect("recall");
    assert_eq!(results.len(), 1);
...
fn recall_full_text_search() {
    ...
    store
        ...
        .expect("b");
    let results = store
        .recall(RecallQuery { text_search: Some("consensus".to_string()), limit: 10, ..Default::default() })
        .expect("recall");
    assert_eq!(results.len(), 1);
