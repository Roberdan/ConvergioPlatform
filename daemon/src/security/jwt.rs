...
//!
//! Each agent receives a signed token at registration containing its name,
//! role, capabilities, and expiry. The daemon validates the signature on
//! every request and extracts the embedded identity for RBAC decisions.
...
/// Daemon-wide JWT signing secret (initialised once at startup).
static JWT_SECRET: OnceLock<Vec<u8>> = OnceLock::new();

/// Initialise the JWT secret. Call once at daemon startup.
/// Falls back to `CONVERGIO_JWT_SECRET` env var, then generates a
/// random ephemeral secret (logged as warning).
    ...
    let _ = JWT_SECRET.set(match secret {
        ...
        None => match std::env::var("CONVERGIO_JWT_SECRET") {
            ...
            _ => {
                ...
                getrandom(&mut buf);
                tracing::warn!(
                    "No JWT secret configured — using ephemeral secret"
                );
                buf.to_vec()
            }
        },
...
/// Portable random fill (uses rand crate already in deps).
...
pub struct AgentClaims {
    /// Unique agent name (e.g. "planner-opus", "executor-codex-1").
    pub sub: String,
    /// Agent role for RBAC.
    pub role: AgentRole,
    /// Granted capabilities (e.g. ["plan:read", "task:update"]).
    pub cap: Vec<String>,
...
/// Error returned by JWT operations.
...
// --- Minimal JWT encode/decode (header.payload.signature) ---
