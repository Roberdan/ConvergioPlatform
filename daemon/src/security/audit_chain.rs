use super::types::{AuditEntry, SecurityError};
use sha2::{Digest, Sha256};
use std::sync::Mutex;

/// Cryptographic audit chain for agent actions.
/// Each entry includes SHA-256 hash of previous entry (tamper detection).
...
impl AuditChain {
    ...
    pub fn record(
        ...
        *id_counter += 1;
        ...
        let entry_data = format!("{id}{agent_id}{action}{target}{timestamp}{params_hash}{prev_hash}");
    ...
    /// Verify chain integrity. Returns the index of the first broken link, if any.
    ...
    /// Query entries by agent, action, or time range.
