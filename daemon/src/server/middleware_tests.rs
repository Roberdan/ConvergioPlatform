...
fn authenticate_no_header_no_token_no_devmode_denies() {
    let result = authenticate(None, None, "/api/test", "GET", None);
...
fn authenticate_jwt_format_detected_by_dots() {
    ...
    let result = authenticate(Some(fake_jwt), None, "/api/test", "GET", None);
...
fn authenticate_legacy_bearer_without_dots() {
    ...
    let result = authenticate(Some(legacy), None, "/api/test", "GET", None);
    assert!(result.is_err() || result.unwrap().is_none());
}
