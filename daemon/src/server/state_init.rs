...
///
/// Runs once on daemon startup. For each project whose path exists on disk,
/// std::fs::canonicalize resolves symlinks so macOS case-sensitivity bugs
/// (e.g. /Users/foo vs /users/foo on HFS+) are fixed transparently.
/// Paths that no longer exist on disk are left unchanged.
fn canonicalize_existing_project_paths(conn: &Connection) {
