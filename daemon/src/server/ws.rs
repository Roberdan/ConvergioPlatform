...
pub async fn ws_dashboard(ws: WebSocketUpgrade, State(state): State<ServerState>) -> Response {
...
pub async fn ws_brain(ws: WebSocketUpgrade, State(state): State<ServerState>) -> Response {
    ws.on_upgrade(move |socket| {
        handle_ws(
            socket,
...
mod tests {
    ...
    fn ws_channel_accepts_realtime_messages() {
        let state = ServerState::new(PathBuf::from("/tmp/test.db"), None);
