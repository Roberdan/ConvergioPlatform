...
async fn api_mesh(State(state): State<ServerState>) -> Result<Json<Value>, ApiError> {
    ...
    for (name, fields) in &peer_conf {
        ...
        if let Some(alias) = fields.get("ssh_alias") {
            hostname_aliases.push(alias.clone());
        }
        if let Some(dns) = fields.get("dns_name") {
            hostname_aliases.push(dns.clone());
        }
...
async fn handle_mesh_action(Query(qs): Query<HashMap<String, String>>) -> Json<Value> {
    ...
    match action.as_str() {
        "add-node" => {
            ...
            let conf_path =
                std::env::var("HOME").unwrap_or_default() + "/.claude/config/peers.conf";
            let entry = format!(
                "\n[{peer}]\nssh_alias={ssh}\nos={os}\ntailscale_ip={ip}\ncapabilities={caps}\nrole={role}\nstatus=active\n"
            );
            match std::fs::OpenOptions::new().append(true).open(&conf_path) {
                Ok(mut f) => {
                    ...
                    Json(
                        json!({"ok": true, "output": format!("Added {peer} ({ip}) to peers.conf")}),
                    )
                }
            ...
            match std::fs::read_to_string(&conf_path) {
                Ok(content) => {
                    ...
                    match std::fs::write(&conf_path, &result) {
                        Ok(_) => Json(
                            json!({"ok": true, "output": format!("Removed {peer} from peers.conf")}),
                        ),
