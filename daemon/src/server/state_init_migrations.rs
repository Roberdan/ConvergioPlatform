pub(super) const MIGRATIONS: &[&str] = &[
    ...
    "CREATE TRIGGER IF NOT EXISTS trg_plans_updated_at_insert AFTER INSERT ON plans FOR EACH ROW WHEN NEW.updated_at IS NULL BEGIN UPDATE plans SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    "CREATE TRIGGER IF NOT EXISTS trg_plans_updated_at_update AFTER UPDATE ON plans FOR EACH ROW WHEN NEW.updated_at IS NULL OR NEW.updated_at = OLD.updated_at BEGIN UPDATE plans SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    "CREATE TRIGGER IF NOT EXISTS trg_tasks_updated_at_insert AFTER INSERT ON tasks FOR EACH ROW WHEN NEW.updated_at IS NULL BEGIN UPDATE tasks SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    "CREATE TRIGGER IF NOT EXISTS trg_tasks_updated_at_update AFTER UPDATE ON tasks FOR EACH ROW WHEN NEW.updated_at IS NULL OR NEW.updated_at = OLD.updated_at BEGIN UPDATE tasks SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    "CREATE TRIGGER IF NOT EXISTS trg_waves_updated_at_insert AFTER INSERT ON waves FOR EACH ROW WHEN NEW.updated_at IS NULL BEGIN UPDATE waves SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    "CREATE TRIGGER IF NOT EXISTS trg_waves_updated_at_update AFTER UPDATE ON waves FOR EACH ROW WHEN NEW.updated_at IS NULL OR NEW.updated_at = OLD.updated_at BEGIN UPDATE waves SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    "CREATE TRIGGER IF NOT EXISTS trg_kb_updated_at_insert AFTER INSERT ON knowledge_base FOR EACH ROW WHEN NEW.updated_at IS NULL BEGIN UPDATE knowledge_base SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    "CREATE TRIGGER IF NOT EXISTS trg_notif_updated_at_insert AFTER INSERT ON notifications FOR EACH ROW WHEN NEW.updated_at IS NULL BEGIN UPDATE notifications SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = NEW.id; END",
    // Backfill updated_at for ALL sync tables — rows without updated_at break replication
    "UPDATE plans SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(COALESCE(completed_at, started_at, created_at, strftime('%Y-%m-%dT%H:%M:%fZ','now')),1,19),'T',' ')) WHERE updated_at IS NULL",
    "UPDATE tasks SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(COALESCE(completed_at, started_at, strftime('%Y-%m-%dT%H:%M:%fZ','now')),1,19),'T',' ')) WHERE updated_at IS NULL",
    "UPDATE waves SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(COALESCE(completed_at, started_at, strftime('%Y-%m-%dT%H:%M:%fZ','now')),1,19),'T',' ')) WHERE updated_at IS NULL",
    "UPDATE knowledge_base SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(COALESCE(created_at, strftime('%Y-%m-%dT%H:%M:%fZ','now')),1,19),'T',' ')) WHERE updated_at IS NULL",
    "UPDATE notifications SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(COALESCE(created_at, strftime('%Y-%m-%dT%H:%M:%fZ','now')),1,19),'T',' ')) WHERE updated_at IS NULL",
    "UPDATE plans SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(updated_at,1,19),'T',' ')) WHERE updated_at IS NOT NULL",
    "UPDATE tasks SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(updated_at,1,19),'T',' ')) WHERE updated_at IS NOT NULL",
    "UPDATE waves SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(updated_at,1,19),'T',' ')) WHERE updated_at IS NOT NULL",
    "UPDATE knowledge_base SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(updated_at,1,19),'T',' ')) WHERE updated_at IS NOT NULL",
    "UPDATE notifications SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', replace(substr(updated_at,1,19),'T',' ')) WHERE updated_at IS NOT NULL",
];
