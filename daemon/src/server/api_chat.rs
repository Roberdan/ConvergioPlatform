...
pub fn router() -> Router<ServerState> {
    Router::new()
        ...
        .route(
            "/api/chat/session",
            post(handle_chat_session_create).delete(handle_chat_session_delete),
        )
...
async fn handle_chat_session_create(
    ...
    let title = payload
        .title
        .unwrap_or_else(|| "New chat session".to_string());
    conn
        .execute(
    ...
    Ok(Json(
        json!({"ok": true, "session": {"id": sid, "title": title, "status": "active"}}),
    ))
}
...
async fn handle_chat_message_create(
    ...
    Ok(Json(
        json!({"ok": true, "message_id": conn.last_insert_rowid()}),
    ))
}
...
async fn handle_chat_requirement_upsert(
    ...
    if sid.is_empty() || key.is_empty() || text.is_empty() {
        return Err(ApiError::bad_request(
            "missing session_id, requirement_key, or requirement_text",
        ));
    }
    conn
        .execute(
    ...
    Ok(Json(
        json!({"ok": true, "requirement_key": key, "action": "upsert"}),
    ))
}
...
async fn handle_chat_session_delete(
    ...
    Ok(Json(
        json!({"ok": true, "deleted": true, "session_id": sid}),
    ))
}
