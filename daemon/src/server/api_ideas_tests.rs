...
fn test_router() -> axum::Router {
    ...
    let tmp = std::env::temp_dir().join(format!("claude-ideas-test-{}-{n}.db", std::process::id()));
...
async fn ideas_crud() {
    ...
    let (s, j) = post(
        &r,
        "/api/ideas",
        serde_json::json!({"title": "New Idea", "priority": "P1", "tags": "test"}),
    )
    .await;
    ...
    let (s, j) = put(
        &r,
        &format!("/api/ideas/{new_id}"),
        serde_json::json!({"title": "Updated Idea", "status": "elaborating"}),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
...
// ideas_notes, ideas_notes_rejects_empty_content, ideas_promote, ideas_filter
// are covered in api_ideas_tests_filter.rs
