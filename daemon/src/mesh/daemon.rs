...
pub(super) struct InboundConnectionRateLimiter {
    minute_limiter: crate::mesh::observability::RateLimiter,
    second_windows: std::sync::Mutex<HashMap<IpAddr, Vec<std::time::Instant>>>,
    max_per_second: usize,
}

impl InboundConnectionRateLimiter {
    pub(super) fn new(max_per_second: usize, max_per_minute: usize) -> Self {
        Self {
    ...
    pub(super) fn check(&self, remote: SocketAddr) -> Result<(), String> {
        let ip = remote.ip();
        {
    ...
    pub(super) fn release(&self, remote: SocketAddr) {
...
pub(super) fn relay_agent_activity_changes(
    state: &DaemonState,
    node: &str,
    changes: &[DeltaChange],
) {
...
pub(super) fn relay_ipc_changes(state: &DaemonState, node: &str, changes: &[DeltaChange]) {
    for change in changes {
    ...
    loop {
        match TcpStream::connect(&peer).await {
            Ok(stream) => {
                ...
                let _ = apply_socket_tuning(&stream);
                let _ = daemon_sync::handle_socket(
                    stream,
                    format!("peer-{peer}"),
                    state.clone(),
                    config.clone(),
