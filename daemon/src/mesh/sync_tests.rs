...
fn crdt_allowlist_blocks_unknown_tables() {
    ...
    let changes = vec![
        DeltaChange {
            ...
            pk: b"id=1".to_vec(),
            cid: "title".to_string(),
            val: Some("OK".to_string()),
            col_version: 1,
            db_version: 10,
            site_id: b"peer1".to_vec(),
            cl: 1,
            seq: 1,
        },
        DeltaChange {
...
fn stress_apply_1000_changes() {
    ...
    .expect("seed");
    let changes: Vec<DeltaChange> = (0..1000)
        .map(|i| DeltaChange {
    ...
    assert!(
        ...
        "1000 changes should apply in <5s, took {elapsed:?}"
    );
}
