...
mod tests {
    ...
    fn hmac_roundtrip_succeeds() {
        ...
        let hmac = compute_hmac(secret, &nonce).expect("valid HMAC secret");
        assert!(verify_hmac(secret, &nonce, &hmac).expect("valid HMAC secret"));
    }

    #[test]
    fn hmac_rejects_wrong_secret() {
        let nonce = generate_nonce();
        let hmac = compute_hmac(b"correct-secret", &nonce).expect("valid HMAC secret");
        assert!(!verify_hmac(b"wrong-secret", &nonce, &hmac).expect("valid HMAC secret"));
    }
    ...
    fn hmac_rejects_wrong_nonce() {
        ...
        let hmac = compute_hmac(secret, &nonce1).expect("valid HMAC secret");
        assert!(!verify_hmac(secret, &nonce2, &hmac).expect("valid HMAC secret"));
    }
    ...
    fn hmac_rejects_empty_response() {
        let secret = b"secret";
        let nonce = generate_nonce();
        assert!(!verify_hmac(secret, &nonce, &[]).expect("valid HMAC secret"));
    }
    ...
    fn hmac_rejects_truncated_response() {
        ...
        let nonce = generate_nonce();
        let hmac = compute_hmac(secret, &nonce).expect("valid HMAC secret");
        assert!(!verify_hmac(secret, &nonce, &hmac[..16]).expect("valid HMAC secret"));
    }
}
