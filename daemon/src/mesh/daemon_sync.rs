...
use super::{
    handle_ws_client, is_ws_brain_request, now_ts, publish_event, relay_agent_activity_changes,
    relay_ipc_changes, DaemonConfig, DaemonState,
};
            ...
            match db_result {
                Ok((changes, checkpoint, effective_cursor)) => {
                    ...
                    if !changes.is_empty() {
                        ...
                        if staged_changes.len() + changes.len() > MAX_STAGED
                        ...
                        if changes.len() > MAX_STAGED {
                            eprintln!("WARN: incoming change batch exceeded {MAX_STAGED}, sending immediately");
                            let send_count = changes.len();
                            let last_db_version = checkpoint;
                            let frame = MeshSyncFrame::Delta {
                                node: node_id.clone(),
                                sent_at_ms: sync::current_time_ms(),
                                last_db_version,
