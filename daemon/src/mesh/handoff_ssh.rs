...
impl SshClient {
    pub fn connect(dest: &str, timeout: Duration) -> Result<Self, MeshError> {
        ...
        session
            ...
            .map_err(|e: ssh2::Error| MeshError::Auth(e.to_string()))?;
        if !session.authenticated() {
            return Err(MeshError::Auth("ssh authentication failed".into()));
        }
        Ok(Self { session })
    }

    pub fn exec(&self, command: &str) -> Result<(i32, String, String), MeshError> {
    ...
    pub fn scp_download(&self, remote: &Path, local: &Path) -> Result<(), MeshError> {
    ...
    pub fn scp_upload(&self, local: &Path, remote: &Path, mode: i32) -> Result<(), MeshError> {
