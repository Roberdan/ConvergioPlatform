//! Internal helpers for repo scanning.
...
pub(crate) const MAX_FILES_FOR_LINES: usize = 1000;
...
fn detect_rust_frameworks(root: &Path, fw: &mut Vec<String>) {
    for cargo_path in &[root.join("Cargo.toml"), root.join("daemon/Cargo.toml")] {
        if let Ok(content) = fs::read_to_string(cargo_path) {
            ...
            if content.contains("rocket") && !fw.contains(&"Rocket".to_string()) {
                fw.push("Rocket".to_string());
            }
        }
    }
}
