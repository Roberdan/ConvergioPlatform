...
/// POST helper with 10 s timeout and optional bearer auth from env.
...
///
/// Steps: create org -> register agents -> create bootstrap plan ->
/// create initial tasks -> schedule night agents.
pub fn provision_org(
    ...
    let plan_resp = api_post(
        &format!("{daemon_url}/api/plan-db/create"),
        &plan_body,
    )?;
    ...
    // 5. Schedule night agents as tasks with schedule metadata
    let mut night_agents_scheduled: usize = 0;
    for na in &blueprint.night_agents {
        let task_body = json!({
            "plan_id": plan_id,
            "title": format!("Night: {} ({} {})", na.name, na.time, na.schedule),
            "status": "pending",
            "metadata": json!({
                "night_agent": true,
