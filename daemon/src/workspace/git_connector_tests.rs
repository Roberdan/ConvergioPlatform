...
fn git_error_api_display() {
    ...
    let msg = e.to_string();
    assert!(
        msg.contains("422") && msg.contains("Unprocessable Entity"),
        "got: {msg}"
    );
}
...
fn git_error_parse_display() {
    assert!(GitError::Parse("bad json".into())
        .to_string()
        .contains("bad json"));
}
...
fn git_error_git_display() {
    assert!(GitError::Git("not a repo".into())
        .to_string()
        .contains("not a repo"));
}
...
async fn connector_create_pr_returns_future() {
    let c = GitHubConnector {
        github_token: "invalid-token".into(),
    };
...
async fn connector_pr_readiness_returns_future() {
    ...
    let result =
        tokio::time::timeout(Duration::from_millis(100), c.pr_readiness("org/repo", 1)).await;
    match result {
        Ok(Err(GitError::Request(_))) | Ok(Err(GitError::Api { .. })) | Err(_) => {}
        Ok(Ok(_)) => panic!("should not succeed with invalid token"),
        Ok(Err(e)) => panic!("unexpected error variant: {e}"),
