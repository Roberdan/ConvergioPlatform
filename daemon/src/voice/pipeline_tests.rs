...
// --- Silence produces no intent ---

#[test]
fn silence_frames_produce_no_intent() {
    let mut p = default_pipeline();
    p.start().unwrap();
    for i in 0..10 {
        let result = p.process_frame(&silence_frame(i * 10)).unwrap();
        assert!(result.is_empty());
    }
    assert_eq!(p.state(), VoiceState::Listening);
}
...
fn pipeline_event_debug_display() {
    ...
    assert!(format!("{ev:?}").contains("WakeDetected"));
}
...
fn pipeline_event_error_variant_captures_message() {
    ...
    assert!(format!("{ev:?}").contains("test error"));
}
