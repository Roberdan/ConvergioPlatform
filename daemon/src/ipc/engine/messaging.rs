...
impl IpcEngine {
    pub fn send_message(
        ...
        // Empty sender breaks rate-limiting and audit trail.
        debug_assert!(!from.is_empty(), "send_message: from must not be empty");
        // Empty recipient causes the message to be delivered to nobody.
        debug_assert!(!to.is_empty(), "send_message: to must not be empty");
        // Empty content is a no-op payload — almost always a caller bug.
        debug_assert!(!content.is_empty(), "send_message: content must not be empty");
        // msg_type drives consumer dispatch; blank type is unroutable.
        debug_assert!(!msg_type.is_empty(), "send_message: msg_type must not be empty");
    ...
    pub fn broadcast(
        ...
        // Broadcast sender must be identified for rate-limiting.
        debug_assert!(!from.is_empty(), "broadcast: from must not be empty");
        // Content must carry a payload — empty broadcast is wasted I/O.
        debug_assert!(!content.is_empty(), "broadcast: content must not be empty");
        // Channel name, when given, must be non-empty to resolve to a valid channel.
        debug_assert!(
            channel.map_or(true, |c| !c.is_empty()),
            "broadcast: channel must be non-empty when provided"
        );
        let conn = self.open_conn()?;
        if let Some(err) = self.check_rate_limit(&conn, from)? {
            return Ok(err);
