...
function showPeerCrudToast(title, message, type = "error") {
  if (typeof showToast === "function") {
    showToast(title, message || "", null, type);
    return;
  }
...
async function savePeer(mode) {
  ...
  try {
    ...
    if (json.error) {
      showPeerCrudToast("Save failed", json.error);
      return;
    }
    closePeerForm();
    if (typeof refreshMesh === "function") refreshMesh();
  } catch (e) {
    showPeerCrudToast("Save failed", e.message);
  }
}
...
async function deletePeer(name) {
  ...
  if (mode === "hard") {
    ...
    if (v !== "ELIMINA") {
      showPeerCrudToast("Delete failed", "Type ELIMINA to confirm hard delete");
      return;
    }
  }
  try {
    ...
    const json = await res.json();
    if (json.error) {
      showPeerCrudToast("Delete failed", json.error);
      return;
    }
    closePeerForm();
    if (typeof refreshMesh === "function") refreshMesh();
  } catch (e) {
    showPeerCrudToast("Delete failed", e.message);
  }
}
  ...
  try {
    ...
    overlay
      ...
      .addEventListener("click", async () => {
          ...
          try {
            const r = await fetch("/api/peers", {
              ...
              body: JSON.stringify({
                ...
                user: window.DashboardState?.currentUser || "",
                os: p.os,
                role: "worker",
        ...
        if (errors.length) {
          showPeerCrudToast("Peer discovery", "Some peers failed:\n" + errors.join("\n"));
        }
        closePeerForm();
        if (typeof refreshMesh === "function") refreshMesh();
      });
