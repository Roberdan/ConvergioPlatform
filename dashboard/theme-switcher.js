 ...
 * Default: Maranello (Ferrari Luce DS). Legacy themes via data-theme attribute.
 * Reads/writes theme to localStorage, renders dropdown, applies data-theme attribute.
 */

const THEMES = [
  { id: 'maranello', name: 'Maranello', sub: 'Ferrari', color: '#FFC72C' },
...
function getTheme() {
  return localStorage.getItem(STORAGE_KEY) || 'maranello';
}
...
function syncThemeToggleState(isOpen) {
...
window.addEventListener('load', () => {
  if (window.maranelloEnhancer) {
    window.maranelloEnhancer.activate().catch(() => {});
  }
});
