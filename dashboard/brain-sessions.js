...
(() => {
  'use strict';
  const PI2 = Math.PI * 2;
  // Tool-agnostic palette: known tools get branded colors, unknown get hash-based
  const SESSION_COL = { claude: { h: 40, base: '#ffb020' }, copilot: { h: 210, base: '#20a0ff' }, opencode: { h: 150, base: '#00e080' } };
  const _sessionColCache = {};
  function sessionCol(tool) {
    if (SESSION_COL[tool]) return SESSION_COL[tool];
    if (_sessionColCache[tool]) return _sessionColCache[tool];
    let hash = 0;
    for (let i = 0; i < tool.length; i++) hash = ((hash << 5) - hash + tool.charCodeAt(i)) | 0;
    const h = ((Math.abs(hash) * 137.508) % 360);
    _sessionColCache[tool] = { h, base: `hsl(${h},75%,55%)` };
    return _sessionColCache[tool];
  }

  function toolFromId(sid) {
    if (sid.includes('copilot')) return 'copilot';
    if (sid.includes('opencode')) return 'opencode';
    if (sid.includes('claude')) return 'claude';
    const m = sid.match(/^session-([a-z]+)/);
