...
DB_PATH = os.path.expanduser(os.environ.get('DASHBOARD_DB', '~/.claude/data/dashboard.db'))
...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def daemon_candidates(self):
        ...
        # Auto-detect tailscale IP from lsof on port 9421
        if not host_list:
            try:
                import subprocess
                out = subprocess.check_output(
                    ['lsof', '-iTCP:9421', '-sTCP:LISTEN', '-n', '-P'],
    ...
    def daemon_request(self, daemon_path, query='', timeout=3, method='GET'):
        ...
        # Reset cache on total failure so next request retries discovery
        DashboardHandler.daemon_base_url = None
    ...
    def get_peers(self):
        return {"peers": [
            {"peer_name": "mac-worker-2", "is_online": True, "load_json": '{"cpu": 45.2, "tasks": 2}'},
            {"peer_name": "linux-worker", "is_online": True, "load_json": '{"cpu": 67.8, "tasks": 2}'},
            {"peer_name": "mac-worker-1", "is_online": True, "load_json": '{"cpu": 23.1, "tasks": 1}'}
        ]}
