...
function quickPreflight(planId, el, btn, target) {
  ...
  const cleanup =
    typeof attachSseCleanup === 'function'
      ? attachSseCleanup(es, el.closest('.modal-overlay'))
      : () => {
          try {
            es.close();
          } catch {}
  ...
  es.addEventListener('result', (e) => {
    clearTimeout(t);
    const d = typeof parseSseJson === 'function' ? parseSseJson(e.data) : null;
    cleanup();
    if (!d) {
      el.innerHTML = 'Could not verify \u2014 start anyway?';
      el.style.color = 'var(--gold)';
  ...
  (async () => {
    try {
      ...
      if (typeof showOutputModal === 'function') {
        showOutputModal('Full Sync Error', err.message || 'Failed to start full sync');
      }
    }
  })();
};
