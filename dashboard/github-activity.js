  ...
  function renderGitHubEvent(evt) {
    ...
    const click = evt.url
      ...
      : evt.planId
        ...
        : "";
    const source = evt.source ? `<span class="event-peer mn-badge mn-badge--neutral">${esc(evt.source)}</span>` : "";
    const plan = evt.planId ? `<span class="event-plan">#${evt.planId}</span>` : "";
    return `<div class="event-row github-activity-row github-activity-${evt.kind} mn-anim-fadeIn"${click} style="cursor:${clickable ? "pointer" : "default"}"><span class="github-activity-icon" style="color:${evt.color}">${evt.icon}</span><span class="event-type mn-tag">${esc(evt.label)}</span>${source}${plan}<span class="event-age">${timeAgo(evt.ts)}</span></div><div class="github-activity-detail">${esc(evt.details)}</div>`;
  }
