...
function _meshNodeHtml(p) {
  ...
  const nodeAccent = nodeCol
    ? nodeCol
    : p.role === 'coordinator'
      ? 'var(--giallo, #FFC72C)'
      : p.is_online
        ? 'var(--signal-ok, #00A651)'
        : 'var(--grigio-2, #444)';
  const borderStyle = (p.is_online && nodeCol)
    ? `border-color:${nodeCol};box-shadow:0 0 14px ${nodeGlow}0.15);`
    : '';
  const nodeStyle = `--mn-mesh-border-accent:${nodeAccent};${borderStyle}`;
  const cls = [
      "mn-mesh-node",
    ...
    c === "claude"
      ...
      : c === "copilot"
        ...
        : c === "ollama"
          ...
          : "";
  return `<div class="${cls}" data-peer="${esc(p.peer_name)}" style="${nodeStyle}">
    <div class="mn-mesh-node__header"><span class="mn-os">${OS_ICON[p.os] || OS_ICON.unknown}</span><span class="mn-mesh-node__name"${nodeCol ? ` style="color:${nodeCol}"` : ''}>${esc(p.peer_name)}</span><span class="mn-mesh-status ${p.is_online ? "mn-mesh-status--on" : "mn-mesh-status--off"}"${nodeCol ? ` style="background:${nodeCol};box-shadow:0 0 6px ${nodeCol}"` : ''}></span></div>
    <div class="mn-mesh-node__role"${nodeCol ? ` style="color:${nodeCol}"` : ''}>${esc((p.role || '').toUpperCase())}${p.is_local ? " · LOCAL" : ""}</div>
    <div class="mn-mesh-badges">${caps.map((c) => `<span class="mn-mesh-badge ${capVariant(c)}">${esc(c)}</span>`).join("")}</div>
    ${p.is_online ? `<div class="mn-mesh-node__stats">${p.active_tasks} tasks · CPU ${Math.round(p.cpu)}%${memTotal > 0 ? ` · RAM ${memUsed.toFixed(1)}/${memTotal.toFixed(0)}GB` : ""}</div><div class="mn-mesh-bar"><span class="mn-mesh-bar__label">CPU</span><span class="mn-mesh-bar__track"><span class="mn-mesh-bar__fill mn-mesh-bar__fill--cpu" style="width:${Math.round(loadPct)}%;background:${loadColor}"></span></span><canvas id="spark-cpu-${peerKey}" class="mn-sparkline" height="28" style="width:100%" data-history="${hist.cpu.join(",")}" data-color="${loadColor}" data-pct="${loadPct}"></canvas><span class="mn-mesh-bar__label" style="color:${loadColor}">${Math.round(loadPct)}%</span></div>${memTotal > 0 ? `<div class="mn-mesh-bar"><span class="mn-mesh-bar__label">RAM</span><span class="mn-mesh-bar__track"><span class="mn-mesh-bar__fill mn-mesh-bar__fill--ram" style="width:${memPct}%;background:${memColor}"></span></span><canvas id="spark-mem-${peerKey}" class="mn-sparkline" height="28" style="width:100%" data-history="${hist.mem.join(",")}" data-color="${memColor}" data-pct="${memPct}"></canvas><span class="mn-mesh-bar__label" style="color:${memColor}">${memPct}%</span></div>` : ""}` : '<div class="mn-mesh-node__stats offline-text">No heartbeat</div>'}
    ${planHtml}${actions}
  </div>`;
}
...
function _drawSpokes() {
  ...
  const wr = c.previousElementSibling,
    ...
    cn = cr && cr.querySelector(".mn-mesh-node"),
    wn = wr && wr.querySelectorAll(".mn-mesh-node");
...
function renderMeshStrip(peers) {
  ...
  if (coord && workers.length) {
    ...
    el.innerHTML = `<div class="mn-mesh-network mn-mesh-network__grid">${left.map(_meshNodeHtml).join("")}<div class="mn-mesh-network__coord">${_meshNodeHtml(coord)}</div>${right.map(_meshNodeHtml).join("")}<canvas class="mesh-flow-canvas" id="mesh-flow-cvs"></canvas></div>`;
  } else
    el.innerHTML = `<div class="mn-mesh-network mn-mesh-network__grid">${peers.map(_meshNodeHtml).join("")}</div>`;
...
function _initMeshFlow() {
  if (_meshFlowRAF) {
    cancelAnimationFrame(_meshFlowRAF);
    _meshFlowRAF = 0;
  }
  ...
  const nodes = hub.querySelectorAll('.mn-mesh-node');
  ...
  function spawnBetween(fromName, toName, count, eventType) {
    ...
    if (_meshFlowParticles.length > 200) return;
  ...
  function connectDaemonWs() {
    ...
    _meshDaemonWs.onmessage = (e) => {
      ...
      try {
        ...
        console.warn('Malformed mesh daemon WS message:', err.message, typeof e.data === 'string' ? e.data.substring(0, 100) : e.data);
        return;
      }
      const kind = msg.kind || '';
      ...
      const fresh = hub.querySelectorAll('.mn-mesh-node');
      if (fresh.length >= 2) {
      ...
      if (kind === 'heartbeat' && localNode && sourceNode !== localNode) {
        ...
        if (localNode && sourceNode !== localNode)
          spawnBetween(sourceNode, localNode, 1, 'heartbeat');
      } else if (kind.startsWith('ipc_')) {
        // Events: ipc_agent_register, ipc_lock_change, ipc_message, ipc_worktree_change
        if (typeof window.handleIpcWsEvent === 'function') window.handleIpcWsEvent(msg);
      }
    };
    _meshDaemonWs.onclose = () => { setTimeout(connectDaemonWs, 5000); };
    _meshDaemonWs.onerror = (err) => {
      console.warn('Mesh daemon WS error:', err);
      if (_meshDaemonWs) _meshDaemonWs.close();
    };
  }
  ...
  function animate(ts) {
    ...
    const liveNodes = hub.querySelectorAll('.mn-mesh-node');
...
function applyMeshSyncBadges(items) {
  ...
  items.forEach((s) => {
    const n = document.querySelector(`.mn-mesh-node[data-peer="${CSS.escape(s.peer_name)}"]`);
    if (!n) return;
    ...
    b.title = title;
    n.querySelector(".mn-mesh-node__header").appendChild(b);
  });
}
...
function connectDashboardWs() {
  ...
  ws.onmessage = (e) => {
    ...
    try {
      ...
      console.warn('Malformed dashboard WS message:', err.message, typeof e.data === 'string' ? e.data.substring(0, 100) : e.data);
      return;
    }
    if (!m) return;
    ...
    if (window.Maranello && typeof window.Maranello.emit === "function") {
      if (m.type === "kpi" && m.data) {
        try {
          window.Maranello.emit("kpi.plans_active", m.data.plans_active ?? 0);
          window.Maranello.emit("kpi.mesh_online", m.data.mesh_online ?? "0/0");
          window.Maranello.emit("kpi.blocked", m.data.blocked ?? 0);
          window.Maranello.emit("kpi.cost", m.data.cost ?? "$0");
        } catch (_) {}
      }
    }
  };
