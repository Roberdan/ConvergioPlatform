...
window.showMovePlanDialog = async function (targetPeer) {
  let plans;
  try {
    plans = await fetchJson("/api/plans/assignable");
  } catch (err) {
    showOutputModal("Move Plan", err.message || "Failed to load assignable plans");
    return;
  }
...
window.movePlan = async function (planId, target, overlay) {
  try {
    ...
    if (overlay) overlay.remove();
    showOutputModal("Move Error", err.message || "Failed");
  }
};
...
window.showDelegatePlanDialog = async function (planId, planName) {
  let raw;
  try {
    raw = await fetchJson("/api/mesh");
  } catch (err) {
    showOutputModal("Delegate Plan", err.message || "Failed to load mesh nodes");
    return;
  }
