...
function updateOptimizeBadge() {
  ...
  if (!badge) return;
  badge.classList.add('mn-badge', 'mn-badge--warning');
...
function renderSignalCard(sig) {
  ...
  const samples = Array.isArray(sig.samples)
    ...
    : '';
  return `<div class="opt-signal-card mn-card-dark mn-hover-lift">
    <div class="opt-signal-header">
      ${icon}
      <span class="opt-signal-label mn-stat__label">${label}</span>
      <span class="opt-signal-count mn-stat__value">${sig.count}</span>
    </div>
...
function openOptimizeModal() {
  ...
  overlay.innerHTML = `
    <div class="modal-box opt-modal mn-card-dark">
      ...
      <div class="opt-actions">
        ${!isEmpty ? `<button class="opt-btn mn-btn mn-btn--sm opt-btn-clear" onclick="clearOptimizeSignals()">Archive & Clear</button>` : ''}
        <button class="opt-btn mn-btn mn-btn--sm opt-btn-refresh" onclick="refreshOptimize()">Refresh</button>
      </div>
    </div>`;
