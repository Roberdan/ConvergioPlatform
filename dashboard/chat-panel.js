(function () {
    ...
    root.innerHTML = `
      <div class="chat-panel">
        ...
        <div class="chat-controls">
          ...
          <select id="chat-model-select" class="chat-select mn-select" aria-label="Model selector"></select>
          <select id="chat-node-select" class="chat-select mn-select" aria-label="Node selector"></select>
          <div id="chat-stream-status" class="chat-status mn-badge mn-badge--neutral">idle</div>
        </div>
        <div id="chat-messages" class="chat-messages"><div class="chat-empty">Start a chat to stream messages.</div></div>
        <form id="chat-form" class="chat-input-row">
          <input id="chat-input" class="chat-input mn-input" type="text" placeholder="Type message and press Enter..." autocomplete="off" />
          <button id="chat-send-btn" class="mn-widget__action mn-btn mn-btn--accent mn-btn--sm chat-send" type="submit">Send</button>
        </form>
      </div>`;
    return true;
  }
  ...
  function renderMessageList(rows) {
    ...
    box.innerHTML = rows
      .map(
        (m) =>
          `<div class="chat-msg mn-anim-fadeIn ${esc(m.role || "assistant")}" data-id="${m.id || 0}">${esc(
            m.content || "",
          )}</div>`,
      )
      .join("");
  ...
  function upsertRow(msg) {
    ...
    row.className = `chat-msg mn-anim-fadeIn ${msg.role || "assistant"}`;
  ...
  function scheduleStream() {
    ...
    if (runtime.reconnectCount >= MAX_RECONNECTS) {
