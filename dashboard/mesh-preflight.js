...
window.delegatePlan = function (planId, targetPeer, planName, cli) {
  ...
  es.addEventListener("phase", (e) => {
    const phase = typeof parseSseJson === "function" ? parseSseJson(e.data) : null;
    if (!phase) return;
  ...
  es.addEventListener("done", (e) => {
    ...
    if (data && data.ok) {
      ...
      output.innerHTML += `<div class="delegate-status-fail" style="padding:12px;margin-top:8px;border:1px solid var(--red);border-radius:4px">${Icons.xCircle(14)} Invalid response</div>`;
    }
    output.scrollTop = output.scrollHeight;
