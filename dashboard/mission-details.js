...
function _progressGradient(pct) {
  if (pct >= 100) return 'var(--verde-racing, #00A651)';
  if (pct >= 75) return 'linear-gradient(90deg, #e6a117, #00cc6a)';
  if (pct >= 50) return 'linear-gradient(90deg, #ee3344, #e6a117)';
  return 'var(--rosso-corsa, #ee3344)';
}
...
function _missionStatusDot(status) {
  return typeof _statusDotWithDs === "function" ? _statusDotWithDs(status) : statusDot(status);
}
  ...
  return `<div class="task-flow"><div class="task-flow-id">${esc(t.task_id || "")}</div><div class="task-flow-agent ${agentCls}"><span class="task-flow-agent-icon">${isCopilot ? "&#9883;" : "&#9672;"}</span>${esc(agentLabel)}${model ? ` <span class="task-flow-model">${esc(model)}</span>` : ""}</div><div class="task-flow-pipe">${steps
    .map((s, i) => {
      const ai = s.key === active,
        ...
        dsCls = ai ? "mn-status-in-progress" : pi ? "mn-status-done" : "mn-status-pending";
      return `<div class="flow-step ${cls} ${dsCls}"><div class="flow-dot"></div><div class="flow-label">${s.label}</div></div>${i < steps.length - 1 ? `<div class="flow-conn ${pi ? "conn-done" : ai ? "conn-active" : ""}"></div>` : ""}`;
    })
    .join("")}</div></div>`;
}
  ...
  const rows = waves
    .map((w) => {
      const total = Number(w.tasks_total || 0),
        ...
        rowCls = statusCls === "done" ? "mn-row-done" : statusCls === "in_progress" ? "mn-row-in-progress" : "mn-row-pending",
        name = w.name ? ` — ${w.name}` : "";
      return `<div class="wave-gantt-row ${rowCls}" style="height:${rowHeight}px"><div class="wave-gantt-bar wave-gantt-${esc(statusCls)} ${rowCls}" style="height:${barHeight}px;width:${width}%"><span><strong>${esc(w.wave_id)}</strong>${esc(name)}</span></div></div>`;
    })
    .join("");
  const arrows = waves
    .map((w, i) => {
...
function renderWaveGantt(waves, p) {
  ...
  activeWaves.forEach((w) => {
      ...
      cls =
        w.status === "done" || w.status === "merging"
          ...
          : w.status === "in_progress"
            ? "in_progress"
            : "pending",
      rowCls = cls === "done" ? "mn-row-done" : cls === "in_progress" ? "mn-row-in-progress" : "mn-row-pending";
    ...
    html += `<div class="wave-row ${rowCls}"><div class="wave-label">${_missionStatusDot(w.status)} <strong>${esc(w.wave_id)}</strong><span class="wave-name">${esc(wName)}</span>${wTheme}</div><div class="wave-bar"><div class="wave-fill ${cls}" style="width:${wPct}%;background:${wg.gradient}"></div></div><div class="wave-pct" style="color:${wg.color}">${wPct}%</div>${wElapsed ? `<div class="wave-elapsed">${wElapsed}</div>` : '<div class="wave-elapsed"></div>'}<div style="margin-left:4px">${thorIcon(w.validated_at)}</div></div>`;
  });
  if (pendingWaves.length > 0) {
    const cid = `pending-waves-${p.id}`;
    html += `<div class="wave-row wave-pending-summary mn-row-pending" onclick="event.stopPropagation();document.getElementById('${cid}').classList.toggle('expanded')"><div class="wave-label">${_missionStatusDot("pending")} ${pendingWaves.length} waves pending</div><div class="wave-expand-icon">&#9662;</div></div>`;
    html += `<div id="${cid}" class="wave-pending-collapse">`;
    pendingWaves.forEach((w) => {
      const wName = w.name ? ` — ${(w.name || "").substring(0, 35)}` : "";
      html += `<div class="wave-row wave-row-dim mn-row-pending"><div class="wave-label">${_missionStatusDot("pending")} <strong>${esc(w.wave_id)}</strong><span class="wave-name">${esc(wName)}</span></div><div class="wave-bar"><div class="wave-fill pending" style="width:0%"></div></div><div class="wave-pct">0%</div></div>`;
    });
    html += "</div>";
  }
