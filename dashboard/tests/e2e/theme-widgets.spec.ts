...
test.describe('Theme Switcher', () => {
  ...
  test('dropdown shows all 13 themes', async ({ page }) => {
    await page.locator('#theme-toggle').click();
...
test.describe('Widget Drag & Drop', () => {
  ...
  test('widgets have IDs for layout persistence', async ({ page }) => {
    const widgets = page.locator('.mn-widget[id]');
  ...
  test('widget headers serve as drag handles', async ({ page }) => {
    const headers = page.locator('.mn-widget__header');
...
test.describe('Empty State Handling', () => {
  ...
  test('empty mesh shows no nodes', async ({ page, mockApis }) => {
    ...
    await page.goto('/');
    await page.waitForTimeout(800);
    await expect(page.locator('.mn-mesh-node')).toHaveCount(0);
  });
