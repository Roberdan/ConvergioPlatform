...
function _kpiModal(title, bodyHtml) {
  ...
  overlay.innerHTML = `<div class="modal-box mn-card-dark">
...
window.openPlansModal = async function () {
  ...
  const rows = plans
    .map(
      (p) =>
        `<div class="kpi-modal-row mn-hover-lift" onclick="closeSidebar && openPlanSidebar(${p.id}); document.querySelector('.kpi-modal-overlay')?.remove();" style="cursor:pointer">
...
window.openAgentsModal = function () {
  ...
  const rows = running
    .map(
      (t) =>
        `<div class="kpi-modal-row mn-hover-lift">
...
window.openCostModal = async function () {
  ...
  const rows = models
    .map((m) => {
      ...
      return `<div class="kpi-modal-row mn-hover-lift">
    ...
    .join("");
  const totalRow = `<div class="kpi-modal-row mn-hover-lift" style="border-top:1px solid var(--border);margin-top:4px;padding-top:4px">
