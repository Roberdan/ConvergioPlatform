...
def test_plan_start_sse_claims_local_with_canonical_peer(tmp_path):
    ...
    with (
        ...
        patch("api_plans_sse.local_peer_name", return_value="mac-worker-2"),
    ...
    conn.close()
    assert row == ("doing", "mac-worker-2")
    assert ("log", "✓ Plan claimed by mac-worker-2") in handler.events
...
def test_reverse_sync_restores_canonical_local_peer_name():
    ...
    with (
        ...
        patch("mesh_handoff_sync.local_peer_name", return_value="mac-worker-2"),
        patch("mesh_handoff_sync.resolve_host_to_peer", return_value="mac-worker-2"),
    ):
        ok, _ = reverse_sync("linux-worker", 99, logs.append)

    assert ok is True
    assert any(
        "UPDATE plans SET execution_host='mac-worker-2' WHERE id=99;" in stmt
