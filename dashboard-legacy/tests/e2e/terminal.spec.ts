...
test.describe('Terminal Manager', () => {
  ...
  test('terminal tabs show correct label', async ({ page }) => {
    await stubWS(page);
    await evalTerm(page, `termMgr.open('linux-worker', 'linux-worker')`);
    await page.waitForSelector('.term-tab', { timeout: 2000 });
    await expect(page.locator('.term-tab').first()).toContainText('linux-worker');
  });
  ...
  test('multiple tabs can be created', async ({ page }) => {
    ...
    await expect(tabs.nth(1)).toContainText('Linux-Worker');
  });
  ...
  test('tmux session label shows in tab', async ({ page }) => {
    await stubWS(page);
    await evalTerm(page, `termMgr.open('linux-worker', 'linux-worker', 'Convergio')`);
    await page.waitForSelector('.term-tab', { timeout: 2000 });
    await expect(page.locator('.term-tab').first()).toContainText('linux-worker [Convergio]');
  });
});
