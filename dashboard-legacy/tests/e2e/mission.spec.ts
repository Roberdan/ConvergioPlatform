...
test.describe('Active Missions & Task Pipeline', () => {
  ...
  test('mission plan shows project badge', async ({ page }) => {
    await expect(page.locator('#mission-content .badge-project').first()).toContainText('VirtualBPM');
  });

  test('mission summary is displayed', async ({ page }) => {
    await expect(page.locator('#mission-content .mission-summary').first()).toContainText('JWT token rotation');
  });
  ...
  test('task row shows status dot and thor icon', async ({ page }) => {
    ...
    await expect(t1Row.locator('.mn-status-dot--success')).toHaveCount(1);
    await expect(t1Row.locator('svg[fill="#00cc55"]')).toHaveCount(1); // Thor validated

    // T6 is blocked
    const t6Row = page.locator('tr[data-task-id="T6"]');
    await expect(t6Row.locator('.mn-status-dot--danger')).toHaveCount(1);
  });
