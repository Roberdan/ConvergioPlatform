/**
 * IPC coordination view — 5 tabs: Agents, Budget, Models, Skills, Locks.
...
const STYLE_ID = 'mn-ipc-view-style';
function injectStyles() {
  if (document.getElementById(STYLE_ID)) return;
  const s = document.createElement('style');
  s.id = STYLE_ID;
  s.textContent = `
    .ipc-gauges{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:1rem;padding:1rem 0}
    .ipc-gauge{text-align:center}
    .ipc-gauge .mn-text--sm{color:var(--mn-text-muted)}
    .ipc-conflicts{padding:1rem 0}
    .ipc-conflict{padding:.5rem;margin-bottom:.5rem;border-left:3px solid var(--signal-danger);background:var(--mn-surface-raised);border-radius:4px}
    .ipc-loading{color:var(--mn-text-muted);padding:1rem}
    .ipc-error{color:var(--signal-danger);padding:.5rem}
    .ipc-empty{color:var(--mn-text-muted);padding:1rem}`;
  document.head.appendChild(s);
}

function esc(s) { const d = document.createElement('div'); d.textContent = s; return d.innerHTML; }
function loading(el, what) { el.innerHTML = `<div class="ipc-loading">Loading ${what}...</div>`; }
function error(el, msg) { el.innerHTML = `<div class="ipc-error">${esc(msg)}</div>`; }
function empty(el, msg) { el.innerHTML = `<div class="ipc-empty">${msg}</div>`; }
...
/** Fetch + render a simple table tab. Reduces boilerplate for tabs 1/3/4. */
async function renderSimpleTab(tab, label, fetchFn, columns, listKey) {
  loading(tab, label);
  const result = await fetchFn();
  if (result instanceof Error || (listKey === 'agents' && !result?.ok)) {
    error(tab, `Failed to load ${label}`);
    console.warn(`[ipc] fetch ${label} failed`, result);
    return;
  }
  const rows = result?.[listKey] || [];
  if (rows.length === 0) { empty(tab, `No ${label} registered.`); return; }
  tab.innerHTML = '';
  tab.appendChild(mkTable(columns, rows));
}

const AGENT_COLS = [
  { key: 'agent_id', label: 'Agent ID' }, { key: 'host', label: 'Host' },
  { key: 'status', label: 'Status' }, { key: 'current_task', label: 'Current Task' },
  { key: 'last_heartbeat', label: 'Last Heartbeat' },
];
const MODEL_COLS = [
  { key: 'id', label: 'ID' }, { key: 'name', label: 'Name' },
  { key: 'provider', label: 'Provider' }, { key: 'context_window', label: 'Context Window' },
  { key: 'cost_per_token', label: 'Cost/Token' },
];
const SKILL_COLS = [
  { key: 'name', label: 'Name' }, { key: 'agent', label: 'Agent' },
  { key: 'version', label: 'Version' }, { key: 'enabled', label: 'Enabled' },
];
const ROUTE_COLS = [
  { key: 'timestamp', label: 'Time' }, { key: 'model', label: 'Model' },
  { key: 'provider', label: 'Provider' }, { key: 'tokens', label: 'Tokens' },
  { key: 'cost_usd', label: 'Cost ($)' },
];
const LOCK_COLS = [
  { key: 'file_pattern', label: 'File Pattern' }, { key: 'agent', label: 'Agent' },
  { key: 'host', label: 'Host' }, { key: 'locked_at', label: 'Locked At' },
];

// -- Tab 2: Budget (gauges + route history) --

async function renderBudgetTab(tab) {
  loading(tab, 'budget');
  const [budgetRes, routeRes] = await Promise.allSettled([fetchIpcBudget(), fetchIpcRouteHistory()]);
  tab.innerHTML = '';
  ...
  if (budgetRes.status === 'fulfilled' && !(budgetRes.value instanceof Error)) {
    ...
    if (budgets.length > 0) {
      ...
      grid.className = 'ipc-gauges';
      for (const b of budgets) {
        const cell = document.createElement('div');
        cell.className = 'ipc-gauge';
        ...
        if (b.alert) { info.style.color = 'var(--signal-warning)'; info.textContent += ` [${b.alert}]`; }
    ...
    error(tab, 'Failed to load budget data');
  }
...
// -- Tab 5: Locks + conflict warnings --

async function renderLocksTab(tab) {
  loading(tab, 'locks');
  const [lockRes, conflictRes] = await Promise.allSettled([fetchIpcLocks(), fetchIpcConflicts()]);
  tab.innerHTML = '';
  ...
  if (lockRes.status === 'fulfilled' && !(lockRes.value instanceof Error)) {
    ...
    else empty(tab, 'No active locks.');
  } else {
    error(tab, 'Failed to load locks');
  }
...
export default function ipc(container, { api, store }) {
  injectStyles();
  container.innerHTML = '';
